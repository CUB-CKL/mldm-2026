...
\begin{frame}[fragile]
\frametitle{Robust AE}
\textbf{Key idea:} anomalies are poorly reconstructed.

\begin{align*}
  \mathcal{L} = \sum_i \left( d\left(e(x_i)\right) + s_i - x_i \right)^2 + \lambda \sum_i \|s_i\|_1\to \min
\end{align*}
\begin{itemize}
  \item $e(\cdot)$, $d(\cdot)$ --- auto-encoder;
  \item $s_i$ --- sparse residuals.
\end{itemize}
...
\begin{frame}[fragile]
\frametitle{Iterative AE}
\textbf{Key idea:} reduce weights of anomalous samples.
